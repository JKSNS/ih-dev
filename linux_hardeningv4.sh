...
function yippie {
    ...
    for (( i=0; i<${#input}; i++ )); do
        ...
        ascii=$(printf "%d" "'$char")
        ascii=$((ascii + 3))
        transformed+=$(printf "\\$(printf '%03o' "$ascii")")
    done

    if [ "$transformed" == "|hhkdz" ]; then
        return 0
    else
        return 1
    fi
...
if ! yippie "$1"; then
    echo "[X] Missing or incorrect flag. Exiting."
    exit 1
fi

echo "[*] Correct flag provided. Continuing with hardening process..."
