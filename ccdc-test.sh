...
########################################################################
# FUNCTION: allow_running_services
########################################################################
function allow_running_services {
    print_banner "Allowing Traffic for Running Services"
    # Get unique listening ports from ss (ignoring header)
    ports=$(ss -lntu | awk 'NR>1 {split($4, a, ":"); print a[length(a)]}' | sort -u)
    for port in $ports; do
        if [ -n "$port" ]; then
            echo "[*] Allowing inbound TCP and UDP traffic on port $port for running services"
...
function setup_custom_iptables {
    ...
    # Automatically allow traffic for currently running services
    allow_running_services
}
...
# In Ansible mode, the backup section is skipped.
function backups {
    if [ "$ANSIBLE" == "true" ]; then
...
# Modified harden_web: In Ansible mode, skip secure MySQL installation and immutability steps.
function harden_web {
...
#!/bin/bash
open_ports=$(ss -lnt | awk 'NR>1 {print $4}' | awk -F':' '{print a[length(a)]}' | sort -u)
...
function main {
    ...
    if [ "$ANSIBLE" != "true" ]; then
         ...
         echo "[*] Ansible mode: Running web and advanced hardening non-interactively."
         harden_web
         advanced_hardening
    fi
